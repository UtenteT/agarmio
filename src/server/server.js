...
// file per i settaggi del gioco
var c = require('../../config.json');

// funzioni di utility
var util = require('./lib/util');

// struttura dati quadtree
var quadtree = require('simple-quadtree');
//il quadtreee gestisce il campo di gioco
...
io.on('connection', function (socket) {
    console.log('utente connesso', socket.handshake.query.type);
    ...
    socket.on('gotit', function (player) {
        ...
        if (util.findIndex(users, player.id) > -1) {
            ...
            console.log('Totale giocatori in linea: ' + users.length);
        }
...
function tickPlayer(currentPlayer) {
    ...
    function collisionCheck(collision) {
        if (collision.aUser.mass > collision.bUser.mass * 1.1  && collision.aUser.radius > Math.sqrt(Math.pow(collision.aUser.x - collision.bUser.x, 2) + Math.pow(collision.aUser.y - collision.bUser.y, 2))*1.75) {
            console.log('[DEBUG] kill: ' + collision.bUser.id);
...
/*=============ROBA IP: NON TOCCARE ===============magia==============magia=============*/
