...
var socket;
var reason;
//funzione per il debugging: controllare sulla console del browser
...
function startGame(type) {
    //qualunque cosa abbia inserito l'utente, non accetto più di 25 caratteri, e tolgo caratt. speciali
...
// controlla la validità del nickname usato.
function validNick() {
    var regex = /^\w*$/;
...
window.onload = function() {
    //due bottoni: uno per iniziare e uno per fare da spettatore
    ...
    btn.onclick = function () {
        ...
        // controlla validità del nickname e inizia la partita
        if (validNick()) {
...
//aggiungo anche la possibilità di iniziare usando "invio"
...
function setupSocket(socket) {
    ...
    socket.on('welcome', function (playerSettings) {
        ...
        window.chat.addSystemLine('Connesso al gioco!');
        window.chat.addSystemLine('Scrivi <b>-help</b> per la lista dei comandi.');
    ...
    socket.on('playerDied', function (data) {
        window.chat.addSystemLine('{GAME} - la cellula <b>' + (data.name.length < 1 ? 'anonima' : data.name) + '</b> è stata mangiata.');
    });

    socket.on('playerDisconnect', function (data) {
        window.chat.addSystemLine('{GAME} - la cellula <b>' + (data.name.length < 1 ? 'anonima' : data.name) + '</b> si è disconnessa.');
    });

    socket.on('playerJoin', function (data) {
        window.chat.addSystemLine('{GAME} - la cellula <b>' + (data.name.length < 1 ? ' anonima' : data.name) + '</b> si è aggiunta.');
    });

    socket.on('leaderboard', function (data) {
        leaderboard = data.leaderboard;
        var status = '<span class="title">Top10</span>';
        for (var i = 0; i < leaderboard.length; i++) {
            ...
            if (leaderboard[i].id == player.id){
                if(leaderboard[i].name.length !== 0)
                    ...
                    status += '<span class="me">' + (i + 1) + ". Cellulina qualunque</span>";
            } else {
                if(leaderboard[i].name.length !== 0)
                    status += (i + 1) + '. ' + leaderboard[i].name;
                else
                    status += (i + 1) + '. Cellulina qualunque';
            }
        }
...
function gameLoop() {
    if (global.died) {
        ...
        if (global.kicked) {
            if (reason !== '') {
                graph.fillText('Cacciato per:', global.screenWidth / 2, global.screenHeight / 2 - 20);
                graph.fillText(reason, global.screenWidth / 2, global.screenHeight / 2 + 20);
            }
            else {
                graph.fillText('Cacciato!', global.screenWidth / 2, global.screenHeight / 2);
            }
        }
        else {
              graph.fillText('Disconnesso!', global.screenWidth / 2, global.screenHeight / 2);
        }
    }
}
